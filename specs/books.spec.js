import { BookService, AuthService, config, UserService, UserBookService, } from '../framework'
import { books } from '../framework/fixtures/Books.json'
  ...
  describe('Books', () => {
    ...
    it('Попытка добавить книгу в коллекцию к пользователю, которая уже есть', async () => {
      ...
      expect(responseAddListOfBooks.data.message).toBe('ISBN already present in the User\'s Collection!')   
      })
    ...
    it('Попытка получить информацию o книге, которой нет у пользователя', async () => {
      ...
      it('Попытка удалить книгу у пользователя, которой у него нет', async () => {
        ...
        expect(responseRemoveOneBook.data.message).toBe('ISBN supplied is not available in User\'s Collection!')
    })
